...
def generate_launch_description():
    ...
    subscriber_params_file = os.path.join(
        pkg_share, 
        'config', 
        'sine_wave_subscriber_params.yaml'
    )
    
    # Verify files exist
    if not os.path.exists(publisher_params_file):
        raise FileNotFoundError(f"Publisher params file not found: {publisher_params_file}")
    if not os.path.exists(subscriber_params_file):
        raise FileNotFoundError(f"Subscriber params file not found: {subscriber_params_file}")
